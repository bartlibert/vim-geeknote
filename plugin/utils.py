...
KeepNeovimMode = False
if int(vim.eval('exists("g:KeepNeovimMode")')):
    KeepNeovimMode = vim.eval("g:KeepNeovimMode")


def createTempFile(**kwargs):
    if "prefix" not in kwargs:
        kwargs["prefix"] = "__Keep__"

    if "suffix" not in kwargs:
        geeknoteformat = "markdown"
        if int(vim.eval('exists("g:KeepFormat")')):
            geeknoteformat = vim.eval("g:KeepFormat")
        kwargs["suffix"] = ".{}".format(geeknoteformat)

    if "dir" not in kwargs:
        if int(vim.eval('exists("g:KeepScratchDirectory")')):
            kwargs["dir"] = vim.eval("g:KeepScratchDirectory")
